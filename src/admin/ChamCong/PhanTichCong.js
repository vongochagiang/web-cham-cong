...
function PhanTichCong() {
  ...
  const [selectedStore, setSelectedStore] = useState('280-HCM-PML');
  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [employees, setEmployees] = useState([]); // Trạng thái cho danh sách nhân viên

  // Hàm lấy dữ liệu nhân viên từ server
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await fetch('http://localhost:3001/api/phantichcong'); // Đường dẫn API cần lấy dữ liệu
        const data = await response.json();
        // console.log(data)
        setEmployees(data.data); // Cập nhật danh sách nhân viên
      } catch (error) {
        console.error('Lỗi khi lấy dữ liệu nhân viên:', error);
      }
    };

    fetchEmployees(); // Gọi hàm khi component được render
  }, []);
          ...
          <table>
            <thead>
              <tr>
                <th colSpan="1">Thông tin nhân viên</th>
                <th colSpan="4">Lịch làm việc</th>
                <th colSpan="1">Dữ liệu được phân tích</th>
                <th>Giờ làm</th>
                <th>Dữ liệu chấm công</th>
              </tr>
              <tr>
            ...
            <tbody>
              {employees.map(employee => (
                <tr key={employee.id}>
                  <td>
                    <div className="employee-info">
                      <div className="name">{employee.name_user}</div>
                      <div className="role">{employee.role_user}</div>
                    </div>
                  </td>
